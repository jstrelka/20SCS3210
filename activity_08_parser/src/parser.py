...
def parse(input, grammar, actions, gotos):
    ...
    while True:
        ...
        # get action
        action = actions[(state, int(token))]
        ...
        if action[0] == 's':
            ...
            tree = Tree()
            # tree.data = int(token)
            tree.data = lexeme # lexemes will read better than tokens for now
            trees.append(tree)
        ...
        # "accept" operation
            ...
            # assume that the lhs of the 1st production has the start symbol
